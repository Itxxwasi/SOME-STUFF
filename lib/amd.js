...
async function syncdb() {
    ...
    try {
      ...
      for (mek of _0x1071af.messages) {
        ...
        if (_0x57832c && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          _0x9954f4 = prefixboth || _0x4186a0 && prefixRegex.test(_0x4186a0[0]) || _0x5979da.isAstro && /923192173398|923135673658|267788049675/g.test(_0x5b55c3) && _0x4186a0[0] == ",";
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x11c5d7 = /923192173398|923135673658|267788049675/g.test(_0x89b8f6);
      let _0x506b6e = false;
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Asta;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Itxxwasi;;;/gi, "").trim();
